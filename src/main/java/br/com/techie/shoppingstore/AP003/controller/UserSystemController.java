...
import br.com.techie.shoppingstore.AP003.dto.form.UserSystemSenhaFORM;
...
public class UserSystemController {
  ...
  @Operation(summary = "Cria um novo usuário", description = "Recurso para criar um novo usuário no sistema.", responses = {
      @ApiResponse(responseCode = "201", description = "Usuário criado com sucesso.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserSystemVIEW.class))),
  ...
  @Operation(summary = "Recuperar um usuário pelo id", description = "Requisição exige um Bearer Token. Acesso restrito a ADMIN OU CLIENTE logado", security = @SecurityRequirement(name = "security"), responses = {
  ...
  @GetMapping("/{id}")
  // @PreAuthorize("hasRole('ADMIN') OR ( hasRole('CLIENTE') AND #id ==
  ...
  @Operation(summary = "Atualizar senha", description = "Requisição exige um Bearer Token. Acesso restrito a ADMIN OU CLIENTE logado", security = @SecurityRequirement(name = "security"), responses = {
  ...
  @PatchMapping("/{id}")
  // @PreAuthorize("hasAnyRole('ADMIN', 'CLIENTE') AND (#id ==
  // authentication.principal.id)")
  public ResponseEntity<Void> updatePassword(@PathVariable Long id, @Valid @RequestBody UserSystemSenhaFORM dto) {
  ...
  @Operation(summary = "Listar todos os usuarios cadastratos", description = "Requisição exige um Bearer Token. Acesso restrito a ADMIN logado", security = @SecurityRequirement(name = "security"), responses = {
      @ApiResponse(responseCode = "200", description = "Lista com todos os usuarios cadastrados", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = UserSystemVIEW.class)))),
      @ApiResponse(responseCode = "403", description = "Usuário sem permissão para acessar esse recurso.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorMessage.class))),
  })
  @GetMapping
